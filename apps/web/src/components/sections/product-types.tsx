        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {PRODUCTS.map((p, i) => (
            <Reveal key={p.title} delay={(i % 3) * 0.06}>
              <a
                href="#simulador"
                className="group flex h-full flex-col rounded-md border border-line bg-white p-6 transition duration-base ease-out hover:-translate-y-0.5 hover:shadow-md"
              >
                ...
                <div className="mt-5 flex items-center justify-between border-t border-line pt-4">
                  ...
                  <span className="inline-flex items-center gap-1 text-sm font-semibold text-impulse-600">
                    Simular
                    <IconArrowRight className="h-4 w-4 transition-transform duration-fast group-hover:translate-x-1" />
                  </span>
                </div>
              </a>
            </Reveal>
